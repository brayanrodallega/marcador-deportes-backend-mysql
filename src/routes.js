...
// Route for get one register
routes.post('/:table', async (req, res) => {
    try {
        const [results] = await pool.query
        (`INSERT INTO ${req.params.table} SET ?`, [req.body])
        res.send('Add OK!')
    } catch (error) {
        console.log(error)
    }
...
// Route for update
routes.put('/:table/:field/:id', async (req, res) => {
    try {
        const [results] = await pool.query
        (`UPDATE ${req.params.table} SET ? WHERE ${req.params.field}_id = ?`, [req.body, req.params.id])
        res.send('Updated OK!')
    } catch (error) {
        console.log(error)
    }
...
// Route for delete
routes.delete('/:table/:field/:id', async (req, res) => {
    try {
        const [results] = await pool.query
        (`DELETE FROM ${req.params.table} WHERE ${req.params.field}_id = ?`, [req.params.id])
        res.send('Remove OK!')
    } catch (error) {
        console.log(error)
    }
...
// Route for login user
routes.get('/:table/:email/:clave', async (req, res) => {
    try {
        const [results] = await pool.query
        (`SELECT * FROM ${req.params.table} WHERE usu_email = '${req.params.email}' AND usu_clave = '${req.params.clave}'`)
        res.json(results)
    } catch (error) {
        console.log(error)
    }
...
// Route for list register with limit
routes.get('/:table/:limit', async (req, res) => {
    try {
        const [results] = await pool.query
        (`SELECT t1.eve_id AS sec, date_format(t1.eve_fecha, '%d-%m-%Y') AS fecha, t2.equ_nombre AS equi1, t3.equ_nombre AS equi2, t1.eve_marca1 AS marca1, t1.eve_marca2 AS marca2, t4.dep_nombre AS deporte, t1.eve_descrip AS descrip
        FROM eventos t1
        LEFT JOIN equipos t2 ON t1.equ_equipo1 = t2.equ_id
        LEFT JOIN equipos t3 ON t1.equ_equipo2 = t3.equ_id
        LEFT JOIN deportes t4 ON t1.dep_id = t4.dep_id
